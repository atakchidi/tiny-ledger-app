...
class ListAccountEntriesServiceTest {
    ...
    private val cash = Account.internal(AccountRole.CASH, eur, clock).also(accounts::save)
    ...
    private fun history(after: Uuid? = null, limit: Int = 50) =
        service.execute(ListAccountEntries(alice.id.toString(), CursorDto(after, limit)))
    ...
    fun `lists movements in the order they were recorded`() {
        ...
        assertEquals(movements.map { it.id.toString() }, history().items.map { it.id })
    }
    ...
    fun `keeps another holder's movements out`() {
        ...
        assertEquals(listOf(hers.id.toString()), history().items.map { it.id })
    }
    ...
    fun `walks the history a page at a time`() {
        ...
        assertEquals(movements.take(2).map { it.toString() }, firstPage.items.map { it.id })
        assertEquals(movements.drop(2).take(2).map { it.toString() }, secondPage.items.map { it.id })
        assertEquals(movements.drop(4).map { it.toString() }, lastPage.items.map { it.id })
    ...
    fun `refuses an id that names no account`() {
        assertFailsWith<AccountNotFound> { service.execute(ListAccountEntries("not-an-account")) }
    }
