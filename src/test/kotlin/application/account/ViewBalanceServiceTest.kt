...
class ViewBalanceServiceTest {
    ...
    fun `shows nothing owed on a fresh account`() {
        assertEquals("0.00", service.execute(ViewBalance(alice.id.toString())).amount)
    }
    ...
    fun `shows a balance the holder has overdrawn`() {
        ...
        assertEquals("-2.50", service.execute(ViewBalance(alice.id.toString())).amount)
    }
    ...
    fun `shows the amount in the currency's own precision`() {
        ...
        assertEquals("1000", service.execute(ViewBalance(yuki.id.toString())).amount)
    }
